...
namespace ImageTools.IO.Gif
{
    ...
    /// </summary>
    public class GifDecoder : IImageDecoder, IStreamingImageDecoder
    {
        #region Constants
        ...
        public void Decode(ExtendedImage image, Stream stream)
        {
            ...
            if (this._logicalScreenDescriptor.GlobalColorTableFlag)
            {
                ...
                stream.SafeRead(_globalColorTable, 0, _globalColorTable.Length);
            }

            int nextFlag = stream.SafeReadByte();
            while (nextFlag != -1)
            {
                ...
                nextFlag = stream.SafeReadByte();
            }
        }

        public IEnumerable<ImageBase> DecodeStream(ExtendedImage image, Stream stream)
        {
            _image = image;
        ...
        private void ReadGraphicalControlExtension()
        {
            ...
            _stream.SafeRead(buffer, 0, buffer.Length);
        ...
        private GifImageDescriptor ReadImageDescriptor()
        {
            ...
            int count = _stream.SafeRead(buffer, 0, buffer.Length);

            if (count != 9)
            {
                Debug.WriteLine("Descriptor: " + count + " ouf of " + 9);
            }
        ...
        private void Skip(int length)
        {
            ...
            while ((flag = _stream.SafeReadByte()) != 0)
            {
                _stream.Seek(flag);
        ...
        private void ReadComments()
        {
            ...
            while ((flag = _stream.SafeReadByte()) != 0)
            {
                ...
                _stream.SafeRead(buffer, 0, flag);
        ...
        private void ReadFrame()
        {
            ...
            int blockSize = _stream.SafeReadByte();

            if (blockSize > 0)
            {
                _stream.Seek(blockSize);
            }
        }

        private ImageBase ReadCurrentFrame()
        {
            GifImageDescriptor imageDescriptor = ReadImageDescriptor();
            ...
            var image = ReadCurrentFrameColors(indices, colorTable, imageDescriptor);
        ...
        {
            int dataSize = _stream.SafeReadByte();

            var lzwDecoder = new LZWDecoder(_stream);
        ...
        private byte[] ReadFrameLocalColorTable(GifImageDescriptor imageDescriptor)
        {
            ...
            if (imageDescriptor.LocalColorTableFlag)
            {
                ...
                _stream.SafeRead(localColorTable, 0, localColorTable.Length);
            }
        ...
        private void ReadFrameColors(byte[] indices, byte[] colorTable, GifImageDescriptor descriptor)
        {
            ...
            for (int y = descriptor.Top; y < descriptor.Top + descriptor.Height; y++)
            {
                ...
                for (int x = descriptor.Left; x < descriptor.Left + descriptor.Width; x++)
                {
                    ...
                    {
                        if (offset * 4 + 3 > _currentFrame.Length)
                        {
                            break;
                        }
        ...
        private ImageBase ReadCurrentFrameColors(byte[] indices, byte[] colorTable, GifImageDescriptor descriptor)
        {
            int imageWidth = _logicalScreenDescriptor.Width;
            int imageHeight = _logicalScreenDescriptor.Height;
    ...
    {
        public static void Seek(this Stream stream, int length)
        {
            if (stream.CanSeek)
            {
        ...
        public static int SafeRead(this Stream stream, byte[] buffer, int offset, int length)
        {
            int total = 0;

            do
            {
                total += stream.Read(buffer, offset + total, length - total);
            }
            while (total < length && stream.CanRead);

            return total;
        }

        public static int SafeReadByte(this Stream stream)
        {
